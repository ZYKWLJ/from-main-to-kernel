...
|-|
|1.限制总物理内存不超过16MB|
|2.根据总内存大小，设置缓冲区(内存的一部分)大小|
|3.确定缓冲区后面就是主内存|
|4.判断是否有内存磁盘化技术，有的话，主内存起始地址后移|
...
### 2.2.3 依memory边界判断buffer边界
#### 2.2.3.1 总物理内存定界+缓冲区大小判断
```c
...
#endif
```
这部分代码，显然，是为了**确定main_memory_start的地址**。

第一行说明，main_memory_start的地址，是buffer_memory_end的地址。这也说明了前面的内存架构的正确性，缓冲区紧接着就是物理主内存。

第二行说明，如果定义了RAMDISK，那么main_memory_start的地址，会增加rd_init(main_memory_start, RAMDISK*1024)的大小。这是显然的。

#### 2.2.4.1 何为RAMDISK？

RAMDISK（内存磁盘） 是一种**将一部分物理内存（RAM）模拟成磁盘的技术**。在 Linux 0.11 中，它允许内核在启动阶段**将一个虚拟磁盘（通常包含根文件系统）加载到内存中**，从而**绕过对物理磁盘（如软盘、硬盘）的依赖**，加快系统启动速度或支持无盘系统。
