...
是通过一个一个键盘敲下的代码，这些代码对应的电流逻辑经过解析后，形成了能够控制硬件高低电平组合。

所以你看，计算机中，没有黑魔法，一切都有物理实体。操作系统也不例外。**只不过，我们为了方便，将这些高低电平组合而成的控制逻辑，叫做操作系统。** 并开启了人类统治计算机的序幕。
...
在这些零件里面，有一个大脑，叫做CPU，内存中的操作系统指令会进入CPU中执行，CPU根据指令，对挂载到总线上的其他硬件进行控制。


那么总线上挂载了这么多的硬件，CPU怎么知道该操作哪一个？所以理所当然的，总线上应该有信息唯一的定位要操作的设备，我们称为“地址线”。实际上，总线只是一个总称，总线由以下三条分线组成：
...
这些外设，都是通过总线相连的。不要被总线这个名字吓倒，它就是连接所有设备的物理电路，仅此而已。

我们



我们在书上常看见的计算机由硬件系统、软件系统组成，其实就是
